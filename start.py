...
class ElementManager:
    ...
    def generate_element(self, element1: str, element2: str):
        ...
        try:
            ...
            recipe_text = "\n".join(
                [
                    f"A: {list(pair)[0]}, B: {list(pair)[1]}, C: {element}"
                    for pair, element in self.primitive_recipe.items()
                ]
            )
            ...
            if recipe_check is not True:
                print(
                    f"このレシピは既に存在しています: {element1} -> {element2} -> {recipe_check}"
                )
                return None
...
class GameManager:
    ...
    def __init__(self) -> None:
        ...
        print("--- ルール ---")
        print(
            "あなたは神に任命されました。今からあなたは世界に新しいものを創造していきます。"
        )
        print("二つの要素を掛け合わせて、色々なものを創造しましょう。")
        # 二分の一でeasyかdifficult
        self.goals = (
            self.easy_goals if random.randint(0, 1) == 0 else self.difficult_goals
        )
    ...
    def start(self):
        ...
        while True:
            ...
            if game_turn % 5 == 0:
                ...
                print(
                    Fore.CYAN
                    + f"要素を梱包し、Claudeくんに{self.goal}が可能かを判断してもらいます…"
                    + Style.RESET_ALL
                )
                self._evaluate_goal()
    ...
    def _add_element(self):
        ...
        try:
            print("---")
            print("要素を二つ選択してください。")
            self.selected_elements[0] = list(self.element_manager.object_element)[
                int(input("最初の要素の番号を入力してください: "))
            ]
            self.selected_elements[1] = list(self.element_manager.object_element)[
                int(input("二番目の要素の番号を入力してください: "))
            ]
        ...
        print(
            Fore.CYAN
            + f"選択された要素: {self.selected_elements[0]}と{self.selected_elements[1]}"
            + Style.RESET_ALL
        )
        print("新しい要素を生成中...")
        self.element_manager.generate_element(
    ...
    def _evaluate_goal(self):
        ...
        if is_valid == False:
            ...
            removable_elements = (
                self.element_manager.object_element
                - self.element_manager.primitive_element
            )
            ...
            print(
                Fore.RED
                + f"失敗！挑戦中に{remove_element}を失ってしまいました。"
                + Style.RESET_ALL
            )
        pass
